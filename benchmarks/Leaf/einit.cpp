...
int main(int argc, char* argv[]) {
    ...
    struct HvEnclInitDesc test;
    test.config_address = 0;
    test.sigstruct = 0;
    __asm__ __volatile__("rdtscp" : "=a" (cycles_low), "=d" (cycles_high));
	for (int i = 0; i < TEST_TIME; i++) {
		service.rax = SVSM_FUNCTION(SVSM_SGX_PROTOCOL, SVSM_SGX_EINIT);
        service.rcx = (unsigned long long) &test;
        Ioctl(fd_test, SVSM_EINIT, &service);
	}
